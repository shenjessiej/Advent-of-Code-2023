const test_workflows_input = `xkv{a>2504:A,x<1530:R,A}
...
const test_parts_input = `{x=1243,m=275,a=647,s=591}
...
let parts_acceptance_map = new Map();

function validate(workflow, ratings) {
  if (workflow == "A") {
    parts_acceptance_map.set(ratings, true); 
    return true;
  } 
  if (workflow == "R") {
    parts_acceptance_map.set(ratings, false); 
    return false;
  }

  let curr_rules = map.get(workflow);
  let rule_matched = false;
  curr_rules.forEach((rule) => {
    if (!rule_matched) {
      if (rule.includes(":")) {
        let condition = rule.split(":")[0];
        let next_workflow = rule.split(":")[1];
        if (condition.includes("<")) {
          if (
            ratings.get(condition.split("<")[0]) <
            Number(condition.split("<")[1])
          ) {
            rule_matched = true;
            return validate(next_workflow, ratings);
          }
        }
...
let workflows = test_workflows_input.split("\n");
...
let map = new Map();
workflows.forEach((workflow) => {
  map.set(workflow[0], workflow[1]);
});

let parts = test_parts_input.split("\n");

let total = 0;

parts.forEach((part) => {
  let ratings = part.replace("{", "").replace("}", "").split(",");
  let x = ratings[0].split("=")[1];
  let m = ratings[1].split("=")[1];
  let a = ratings[2].split("=")[1];
  let s = ratings[3].split("=")[1];
  let ratings_map = new Map();
  ratings_map.set("x", Number(x));
  ratings_map.set("m", Number(m));
  ratings_map.set("a", Number(a));
  ratings_map.set("s", Number(s));
  
  validate("in", ratings_map);
  if (parts_acceptance_map.get(ratings_map)) {
    total += Number(x) + Number(m) + Number(a) + Number(s);
  }
