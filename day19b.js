const workflows_input = `xkv{a>2504:A,x<1530:R,A}
...
const test_workflows_input = `px{a<2006:qkq,m>2090:A,rfg}
pv{a>1716:R,A}
lnx{m>1548:A,A}
...
let ans_ranges_arr = [];
let ans_ranges = new Map();
ans_ranges.set("x", [1, 4000]);
ans_ranges.set("m", [1, 4000]);
ans_ranges.set("a", [1, 4000]);
ans_ranges.set("s", [1, 4000]);

function hash(rule, workflow, target) {
  return "[" + rule + "," + workflow[0] + "," + target + "]";
}
function find_accepted_ranges_target(target) {
  if (target == "in") {
    // then stop
    ans_ranges_arr.push([
      ans_ranges.get("x"),
  ...
  workflows.forEach((workflow) => {
    workflow[1].forEach((rule) => {
      if (rule.split(":")[1] == target || rule == target) {
        ...
        find_accepted_ranges(rule, workflow, target);
      }
    });
  });
}

function find_accepted_ranges(rule, workflow, target_workflow) {
  for (let i = workflow[1].indexOf(rule); i >= 0; i--) {
    let condition = workflow[1][i].split(":")[0];
    let target = workflow[1][i].split(":")[1];

    if (condition == target_workflow) {
      //skip to next rule
    } else if (i == workflow[1].indexOf(rule)) {
      if (condition.includes("<")) {
        let spec_name = condition.split("<")[0];
        let spec_mag = Number(condition.split("<")[1]);
        let new_ans_ranges = ans_ranges.get(spec_name);
        new_ans_ranges[1] = Math.min(spec_mag - 1, new_ans_ranges[1]);
        ans_ranges.set(spec_name, new_ans_ranges);
      } else if (condition.includes(">")) {
        let spec_name = condition.split(">")[0];
        let spec_mag = Number(condition.split(">")[1]);
        let new_ans_ranges = ans_ranges.get(spec_name);
        new_ans_ranges[0] = Math.max(spec_mag + 1, new_ans_ranges[0]);
        ans_ranges.set(spec_name, new_ans_ranges);
      }
    } else {
      // rule does not have an A, so we need to skip it
      if (condition.includes("<")) {
        let spec_name = condition.split("<")[0];
        let spec_mag = Number(condition.split("<")[1]);
        let new_ans_ranges = ans_ranges.get(spec_name);
        new_ans_ranges[0] = Math.max(spec_mag, new_ans_ranges[0]);
        ...
        let spec_mag = Number(condition.split(">")[1]);
        let new_ans_ranges = ans_ranges.get(spec_name);
        new_ans_ranges[1] = Math.min(new_ans_ranges[1], spec_mag);
  ...
  find_accepted_ranges_target(workflow[0]);
}

//parse input

let workflows = workflows_input.split("\n");
workflows = workflows.map((line) => {
  return line.split("{");
});
workflows.forEach((workflow) => {
  workflow[1] = workflow[1].replace("}", "").split(",");
});

find_accepted_ranges_target("A");

//find_accepted_ranges(workflows[0][1][2], workflows[0], "A");

let total = 0;
ans_ranges_arr.forEach((ans_range) => {
  let product = 1;
  ans_range.forEach((range) => {
    if (range[1] <= range[0]) {
      product = 0;
    }
...
// find overlapping combinations by comparing each rule to each other rule

/*
let total_overlap = 0;
for (let i = 0; i < ans_ranges_arr.length-1; i++) {
  for (let j = i+1; j < ans_ranges_arr.length; j++) {
    console.log(ans_ranges_arr[i]);
    console.log(ans_ranges_arr[j]);
    let overlap_x = Math.min(ans_ranges_arr[i][0][1], ans_ranges_arr[j][0][1]) - Math.max(ans_ranges_arr[i][0][0], ans_ranges_arr[j][0][0]); 
    ...
    let overlap_m = Math.min(ans_ranges_arr[i][1][1], ans_ranges_arr[j][1][1]) - Math.max(ans_ranges_arr[i][1][0], ans_ranges_arr[j][1][0]); 
    ...
    let overlap_a = Math.min(ans_ranges_arr[i][2][1], ans_ranges_arr[j][2][1]) - Math.max(ans_ranges_arr[i][2][0], ans_ranges_arr[j][2][0]); 
    ...
    let overlap_s = Math.min(ans_ranges_arr[i][3][1], ans_ranges_arr[j][3][1]) - Math.max(ans_ranges_arr[i][3][0], ans_ranges_arr[j][3][0]); 
    ...
    total_overlap += overlap_a*overlap_m*overlap_s*overlap_x; 
  }
}

console.log(total_overlap);

console.log(total- total_overlap); 
console.log(150679712824673 - 167409079868000);
