...
nodes.forEach((node) => {
  let key = node[0].trim();
  let left_right = node[1].trim().replace(" ","").replace("(", "").replace(")", "").split(",");
  map.set(key, left_right);
  if (key.charAt(2) == "A") {
...
start_nodes.forEach((node) => {
  ...
  while (!found) {
    ...
    if (instruction == "L") {
      current_key = map.get(current_key)[0];
    } else {
      current_key = map.get(current_key)[1];
    }

    if (current_key.charAt(2) == "Z") {
      found = true;
    }
